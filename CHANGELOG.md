...
- **Dependencies** — Bumped `@actual-app/api` from `26.8.1` to `26.9.0`.
...
- **Startup welcome message toggle** — Added `WELCOME_MESSAGE_ENABLED` env var (default `false`). When disabled, the bot no longer sends the full welcome message to all authorized users on startup/restart. The `/start` command always works regardless of this setting.

## [1.5.2] - 2026-08-13

### Fixed

- **Rule detection & category self-healing** — Updated `checkRuleConflicts()` to support `field === 'description'` and payee UUID matching (used by Actual Budget's rule engine). Added post-sync category self-healing in `addTransaction()`: if an Actual Budget server rule modifies the transaction category post-sync, the bot detects the override, logs an alert, and automatically restores the category back to what the user selected in Telegram.
